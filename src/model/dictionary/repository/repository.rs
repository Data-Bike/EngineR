...
use crate::model::dictionary::entity::dictionary::DictionaryGroup;
...
impl Repository {
    ...
    fn getDictionaryGroupFromRow(row: PgRow) -> DictionaryGroup {
        DictionaryGroup {
    ...
    fn getDictionaryGroupsFromRows(rows: Vec<PgRow>) -> Vec<DictionaryGroup> {
        let mut res = Vec::<DictionaryGroup>::new();
        for row in rows {
            res.push(Self::getDictionaryGroupFromRow(row));
        }
        res
    }

    pub async fn getDictionaryGroupFromAlias(alias: String) -> Result<DictionaryGroup, RepositoryError> {
        cache_it!(&alias,dictionary_group_by_alias,{
                let res = select_one(
                    "dictionary_group".to_string(),
                    vec!["*".to_string()],
    ...
    pub async fn getDictionaryGroupById(id: String) -> Result<DictionaryGroup, RepositoryError> {
        cache_it!(&id,dictionary_group_by_id,{
                let res = select_one(
                    "dictionary_group".to_string(),
                    vec!["*".to_string()],
